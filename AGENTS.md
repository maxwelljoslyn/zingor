...
Prefer the shared vocabulary over page-specific rules: `.sheet-row` (+ `--between`, `--grab`) for flex list rows, `.striped` on a list container for zebra rows, `.kv-table` / `.data-table` for tables (`.group-striped` to zebra-stripe by `<tbody>` group instead of by row), `.table-scroll` as the wrapper every `<table>` sits in so a wide one scrolls itself instead of spilling off a narrow page, `.subsection` for a titled block within a section, `.section-head` for a heading row that keeps its full-width underline while holding a control (e.g. a Reorder toggle).
