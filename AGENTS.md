...
- When a commit relates to a GitHub issue, reference it in the commit message body (never the headline): write the bare trailer `Refs #X` — nothing else on that line, no explanation appended to it — as the last line of the body, separated by a single blank line from the rest. Any reasoning goes in the body above it. Branches are rebase-merged, so every commit reaches `master` keeping its own message, and GitHub does not annotate a rebased commit with the PR it came from — that trailer is the only path from a `git blame` line back to the discussion.
- **Never write `Closes #X` in a commit message.** Whether the work resolves an issue is a fact about the branch, not about any one commit in it, and a commit claiming closure is wrong the moment another commit follows it. `Closes #X` belongs in the pull request body: it is the one place that both closes the issue on merge and can be corrected afterwards, which a pushed commit message cannot be. (This is about work that goes through a PR. A commit pushed straight to `master` may close what it says it closes.)
- **Every commit must stand on its own**: it builds, and the suite passes at it. Rebase merges land each one on `master` permanently, so a commit that only works once a later commit arrives breaks `git bisect` for good. Keep a change together with the test updates it forces — never batch test fixes into a commit at the end. When history is carved out of a finished working tree rather than developed commit by commit, verify this rather than assuming it (`git rebase --exec` over the new commits, or check out each and run the suite), and say which commits were actually verified.
