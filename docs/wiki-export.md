...
The exported MediaWiki markup is more than plain text. Many parts of the exported data are wrapped in **Zingor microformats** (ZMF), the `zingor-*` HTML class names that you'll need for the [external synchronization](external-synchronization.md) feature. Ability scores, name, race, spells, chosen sage fields, sage studies, sage concentrations, sage abilities, and other syncable fields are already marked up with ZMF for you.
